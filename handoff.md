...
- Account Holder confirmed (2026-07-21) the App Store Connect app record exists and the `app.hyper.mobile` bundle identifier is set up; both users being added as internal TestFlight testers. This satisfies the Rev 69 gate, so the flip was executed.
- Flipped every `com.alexanderroesler.hyper` → `app.hyper.mobile` in code: `capacitor.config.ts` appId, Xcode `PRODUCT_BUNDLE_IDENTIFIER` (Debug + Release), `Info.plist` URL scheme + URL name, `HyperAuthPlugin.swift` (scheme guard + callbackScheme), `src/lib/nativeBridge.ts` `NATIVE_AUTH_CALLBACK_SCHEME` (cascades to redirect URL + Settings WHOOP returnTo), `tests/nativeAuth.test.ts`, and the generated `ios/App/App/capacitor.config.json` (also regenerated by `cap sync`). No old refs remain in code (handoff history entries left intact).
- WHOOP Edge Function `allowedReturnTo` now allowlists `app.hyper.mobile://settings`; **redeployed to Hyper-Dev as whoop-oauth v8** (rollback anchor v7). CLI temporarily linked to Hyper-Dev and restored to production `nnwfaaxmyvqsdnfcdxom` afterward.
- Validation: `npm run ios:sync` regenerated config as `app.hyper.mobile`; npm test 378/378 PASS, lint PASS; **unsigned Xcode Apple-framework build BUILD SUCCEEDED with the new bundle ID** (Xcode 26.6, iOS 26.5).
- Production web Google OAuth untouched (origin-based redirect; only native custom-scheme callbacks changed).
...
- Not done / blockers: bundle-ID flip (awaits Account Holder verification of `app.hyper.mobile`), physical-device matrix + GPS field corpus (needs signing team + iPhones), TestFlight (Account Holder).

### Rev 70 addendum — Hyper-Dev apply + VM staging (owner approved items 2 and 3)

- **whoop-oauth v7 deployed to Hyper-Dev** (`supabase functions deploy whoop-oauth --project-ref nwvgkxqjqihqnjuworqz`). Rollback anchor: v6 (2026-07-17). Smoke: GET /callback with no params → 302 to `$APP_BASE_URL/settings?whoop=error`. All required secrets (WHOOP_CLIENT_ID/SECRET, WHOOP_STATE_SECRET, APP_BASE_URL, SUPABASE_*) confirmed present by name on Hyper-Dev.
- **Migration `20260719230000_add_body_weight_measurements` applied to Hyper-Dev.** Remote migration history was reconciled first with `supabase migration repair`: the 3 ad-hoc bootstrap markers (20260213100000, 20260711093000, 20260717010000) marked reverted, the 14 real local migrations marked applied (history rows only — no SQL replayed; schemas verified present by weeks of live Hyper-Dev testing). Dry-run showed exactly one pending migration before push. Verified via REST: anon key → 200 `[]` (RLS enforced), no key → 401. Rollback: `drop table body_weight_measurements;` + `supabase migration repair --status reverted 20260719230000`.
- The CLI was temporarily linked to Hyper-Dev for this and **restored to production ref `nnwfaaxmyvqsdnfcdxom` afterward** (supabase/.temp restored from backup; user's cli-latest modification intact). Future sessions: Hyper-Dev db work now pushes cleanly from this repo's migration dir after a temporary link.
- **Photo worker staged on the CPX21** (`aross@100.86.141.87`, tailnet `cpx21aross`, SSH key `~/.ssh/codex_vm`): `/home/aross/hyper-deploy/` holds the reviewed worker files from commit `be3f88a8` (sha256 of photo-food-worker.mjs verified identical local↔VM), a filled `photo-worker.env` (Hyper-Dev URL + public anon key, allowlist REQUIRED with placeholder UUIDs, origins `capacitor://localhost` + the Mac tailnet web origin), a hardened systemd unit (dedicated `hyper-photo` system user, HOME=/var/lib/hyper-photo, CLIs from its private npm prefix), and an idempotent `bootstrap.sh`. NOT started: `sudo` on the VM needs a password, so the owner must run `sudo bash /home/aross/hyper-deploy/bootstrap.sh` (creates user, installs files + codex/claude CLIs under the service user, enables the service, adds tailnet-only HTTPS ingress `tailscale serve --https=8443 → 127.0.0.1:8788`, curls /health). After bootstrap: interactive `codex login` / `claude /login` as hyper-photo, set real tester UUIDs in `/etc/hyper/photo-worker.env`, restart service. Tailnet HTTPS certs are enabled (`taileaf222.ts.net`); worker URL for the app will be `https://cpx21aross.taileaf222.ts.net:8443`. Second tester needs a tailnet invite (their iPhone is not on the tailnet) and their own isolated instance per the README — do not pool logins.
